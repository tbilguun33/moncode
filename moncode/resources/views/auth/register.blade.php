...
@section('title', 'Бүртгүүлэх — ' . config('app.name'))
...
<div class="mx-auto flex min-h-[70vh] max-w-md flex-col justify-center px-4 py-12 sm:px-6">
    <div class="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm dark:border-slate-800/80 dark:bg-slate-900/60 dark:shadow-none dark:backdrop-blur-xl">
        <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">Бүртгүүлэх</h1>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Zero-ээс Hero хүртэлх аяллаа эхлүүлээрэй.</p>
        ...
        <form method="POST" action="{{ route('register.store') }}" class="mt-6 space-y-4">
            @csrf
            <div>
                <label for="name" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Нэр</label>
            ...
            <div>
                <label for="password_confirmation" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Нууц үг баталгаажуулах</label>
        ...
        <p class="mt-6 text-center text-sm text-slate-500 dark:text-slate-400">
            Бүртгэлтэй юу?
            <a href="{{ route('login') }}" class="font-medium text-indigo-600 hover:underline dark:text-indigo-400">Нэвтрэх</a>
        </p>
    </div>
</div>
@endsection
